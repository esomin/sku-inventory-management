...
- [x] 10. 프론트엔드 UI 컴포넌트 구현
  - [x] 10.1 SearchFilters 컴포넌트 작성
    - 검색 입력 필드
    - 카테고리 선택 드롭다운
    - "문제 재고 중점만 보기" 체크박스
    - 검색 버튼
  ...
  - [x] 10.3 로딩 및 에러 상태 처리
    - 로딩 인디케이터 표시
    - "No data" 메시지 표시
    - 에러 메시지 표시 (toast 사용)
    - _Requirements: 9.3, 9.4_
  
  - [x] 10.4 SKUForm 컴포넌트 작성 (생성/수정용)
    - 모든 필드 입력 폼
    - 유효성 검증
    - 생성/수정 모드 지원
    - Dialog 또는 Modal로 표시
...
- [x] 13. 엔드투엔드 통합 및 최종 검증
  - [x] 13.1 백엔드와 프론트엔드 동시 실행
